...
WORK_SAMPLE_TASKS = {
    ...
    "comp_labour_stats_041": {2: "Code activity status from a filled labour schedule",
                              3: "Estimate the unemployment rate and its standard error",
                              4: "Reconcile quarterly urban and annual labour estimates"},
    "comp_calibration_102": {2: "Compute non-response adjusted weights for a stratum",
                             3: "Calibrate survey weights to known population totals",
                             4: "Diagnose and trim extreme weights under a variance target"},
    "comp_small_area_101": {2: "Compare direct and synthetic estimates for a district",
                            3: "Fit a Fay-Herriot model for district level estimates",
                            4: "Benchmark model-based small area estimates to state totals"},
    "comp_data_eng_136": {2: "Load a survey extract into a staging table with checks",
                          3: "Build a restartable transformation job for a monthly feed",
                          4: "Diagnose and fix a pipeline that silently drops records"},
    "comp_tabulation_130": {2: "Produce a release table from microdata to the tabulation plan",
                            3: "Check a table set for internal consistency",
                            4: "Automate a full publication table set with quality markers"},
}

# ─────────────────────────────────────────────────────────────────────────────
# FRAC dictionary extension — competencies 041-205 (see domain_extra.py).
#
# The first 40 ids are the original catalogue and keep their content exactly;
# everything below is merged in so the rest of the generator sees one flat
# competency space. CORE_COMPETENCY_IDS remembers which 40 those were.
# ─────────────────────────────────────────────────────────────────────────────
try:                                     # imported as a package (normal case)
    from . import domain_extra as _extra
except ImportError:                      # imported as a top-level module
    import domain_extra as _extra        # type: ignore

CORE_COMPETENCY_IDS = [c[0] for c in COMPETENCIES]

COMPETENCIES += [(cid, name, ctype, decay, desc, topics)
                 for cid, name, ctype, decay, _short, desc, topics, _gs, _ov, _depth
                 in _extra.EXTRA_COMPETENCIES]
SHORT_NAMES.update({e[0]: e[4] for e in _extra.EXTRA_COMPETENCIES})
GSBPM_MAP.update({e[0]: e[7] for e in _extra.EXTRA_COMPETENCIES})
OVERLAP.update({e[0]: list(e[8]) for e in _extra.EXTRA_COMPETENCIES})

# Catalogue depth per competency (A2 at scale): the original 40 subjects carry
# a deep catalogue, the new families a realistic mix. Courses per level:
#   core     2-4    standard  1-3    thin  1-2
CATALOG_DEPTH = {cid: "core" for cid in CORE_COMPETENCY_IDS}
CATALOG_DEPTH.update({e[0]: e[9] for e in _extra.EXTRA_COMPETENCIES})
COURSES_PER_CELL = {
    "core":     {2: 0.50, 3: 0.40, 4: 0.10},
    "standard": {1: 0.35, 2: 0.50, 3: 0.15},
    "thin":     {1: 0.70, 2: 0.30},
}

# Overlaps the original 40 gained once the new families existed — a course on
# the left may legitimately also teach part of the right.
OVERLAP_BACKFILL = {
    "comp_nat_accounts_001": ["comp_io_tables_083", "comp_qna_086", "comp_regional_accounts_085",
                              "comp_deflators_088"],
    "comp_survey_design_002": ["comp_calibration_102", "comp_var_estimation_106", "comp_questionnaire_115",
                               "comp_small_area_101"],
    "comp_price_stats_003": ["comp_producer_price_090", "comp_cost_living_093", "comp_housing_price_091"],
    "comp_index_numbers_004": ["comp_deflators_088", "comp_producer_price_090", "comp_ppp_089"],
    "comp_ml_stats_005": ["comp_mlops_155", "comp_nlp_153", "comp_ai_governance_156"],
    "comp_big_data_006": ["comp_admin_data_125", "comp_web_scraping_158", "comp_spark_137"],
    "comp_econ_census_007": ["comp_business_register_079", "comp_census_pop_078"],
    "comp_dem_analysis_008": ["comp_civil_reg_076", "comp_migration_stats_073"],
    "comp_agri_stats_009": ["comp_agri_census_094", "comp_livestock_stats_095", "comp_horticulture_096"],
    "comp_industry_stats_010": ["comp_business_register_079", "comp_services_stats_060"],
    "comp_gdp_nowcast_011": ["comp_forecasting_112"],
    "comp_spatial_stat_012": ["comp_cv_satellite_154", "comp_frame_maintenance_119"],
    "comp_time_series_013": ["comp_qna_086", "comp_forecasting_112", "comp_benchmarking_113"],
    "comp_poverty_014": ["comp_cost_living_093", "comp_nutrition_stats_046"],
    "comp_sdg_monitor_015": ["comp_gender_stats_047", "comp_water_sanitation_052", "comp_environment_stats_054"],
    "comp_data_gov_016": ["comp_admin_data_125", "comp_nonsampling_120", "comp_metadata_sdmx_123"],
    "comp_python_stats_017": ["comp_data_eng_136", "comp_nlp_153", "comp_reproducible_166"],
    "comp_r_analytics_018": ["comp_econometrics_111", "comp_var_estimation_106"],
    "comp_data_viz_019": ["comp_bi_tools_162", "comp_ux_design_150"],
    "comp_report_writing_020": ["comp_media_relations_184", "comp_tabulation_130"],
    "comp_procurement_021": ["comp_it_procurement_169", "comp_audit_response_176"],
    "comp_rtt_022": ["comp_grievance_178", "comp_records_mgmt_179"],
    "comp_e_gov_023": ["comp_identity_mgmt_147", "comp_records_mgmt_179", "comp_open_data_151"],
    "comp_project_mgmt_024": ["comp_req_analysis_168", "comp_risk_mgmt_189", "comp_survey_ops_126"],
    "comp_public_fin_025": ["comp_gfs_080", "comp_audit_response_176", "comp_gender_budget_186"],
    "comp_data_privacy_026": ["comp_disclosure_105", "comp_cybersecurity_ops_146", "comp_dp_161"],
    "comp_cloud_infra_027": ["comp_containers_143", "comp_network_145"],
    "comp_api_int_028": ["comp_metadata_sdmx_123", "comp_enterprise_arch_170"],
    "comp_db_design_029": ["comp_dwh_139", "comp_data_eng_136", "comp_nosql_140"],
    "comp_statistical_sw_030": ["comp_stata_164", "comp_tabulation_130"],
    "comp_strategic_031": ["comp_systems_think_201", "comp_policy_analysis_188"],
    "comp_leadership_032": ["comp_coaching_196", "comp_feedback_205"],
    "comp_comm_032": ["comp_public_speaking_203", "comp_negotiation_192"],
    "comp_prob_solve_033": ["comp_systems_think_201", "comp_attention_detail_200"],
    "comp_integrity_034": ["comp_accountability_199", "comp_data_ethics_124", "comp_vigilance_173"],
    "comp_change_mgmt_035": ["comp_digital_mindset_202", "comp_resilience_194"],
    "comp_collab_036": ["comp_teamwork_remote_204", "comp_conflict_res_197"],
    "comp_decision_037": ["comp_policy_analysis_188"],
    "comp_innov_038": ["comp_learning_agility_195"],
    "comp_citizen_039": ["comp_grievance_178", "comp_ux_design_150"],
}
for _cid, _extra_overlaps in OVERLAP_BACKFILL.items():
    OVERLAP[_cid] = list(dict.fromkeys(OVERLAP[_cid] + _extra_overlaps))

# Deliberate catalogue holes in the new families. iGOT's real catalogue is
# deepest where demand is: niche subjects often stop at L3/L4, and a few
# specialist subjects have no introductory course at all. Documented in
# data/README.md; the coverage-gap detector and stretch steps run on these.
LADDER_HOLES.update({
    "comp_wage_stats_042":        [5],
    "comp_child_stats_048":       [5],
    "comp_disability_stats_049":  [5],
    "comp_elderly_stats_050":     [5],
    "comp_water_sanitation_052":  [5],
    "comp_forest_stats_056":      [5],
    "comp_transport_stats_057":   [5],
    "comp_mining_stats_063":      [5],
    "comp_insurance_stats_065":   [4, 5],
    "comp_capital_mkt_066":       [5],
    "comp_ecommerce_stats_069":   [5],
    "comp_crime_stats_071":       [5],
    "comp_migration_stats_073":   [5],
    "comp_ppp_089":               [5],
    "comp_service_price_092":     [1],
    "comp_horticulture_096":      [5],
    "comp_mplads_099":            [4, 5],
    "comp_bayesian_107":          [1],
    "comp_exp_design_109":        [5],
    "comp_mixed_mode_117":        [1, 5],
    "comp_nosql_140":             [5],
    "comp_sre_152":               [5],
    "comp_mobile_data_159":       [5],
    "comp_synthetic_data_160":    [1],
    "comp_dp_161":                [1, 5],
    "comp_julia_matlab_165":      [5],
    "comp_enterprise_arch_170":   [5],
    "comp_rajbhasha_172":         [5],
    "comp_pension_175":           [5],
    "comp_event_mgmt_183":        [4, 5],
    "comp_social_media_185":      [5],
    "comp_gender_budget_186":     [5],
    "comp_disaster_mgmt_190":     [5],
})

# ─────────────────────────────────────────────────────────────────────────────
# Four more divisions, so the new families are somebody's day job. Legacy
# departments on the roster seed now fan out across more offices.
# ─────────────────────────────────────────────────────────────────────────────
OFFICES += [
    ("off_ipmd", "Infrastructure & Project Monitoring Division", "New Delhi",
     ["Ministry of Statistics and Programme Implementation (MoSPI)"], ["Infra-Projects", "MPLADS"],
     {"1.1": 3, "5.1": 8, "5.3": 10, "6.1": 10, "6.3": 6, "7.2": 6, "8.1": 4, "OA.PM": 10, "OA.FM": 6},
     {"comp_project_monitor_100": 5, "comp_mplads_099": 4, "comp_construction_stats_062": 2,
      "comp_project_mgmt_024": 2, "comp_public_fin_025": 1}),
    ("off_dsl", "Data Science & Emerging Technology Lab", "Bengaluru",
     ["IT & Data Management Division"], [],
     {"1.5": 6, "3.2": 12, "3.5": 4, "5.1": 8, "5.2": 8, "5.4": 8, "6.1": 5, "8.2": 5, "OA.DM": 6},
     {"comp_ml_stats_005": 4, "comp_nlp_153": 3, "comp_cv_satellite_154": 3, "comp_big_data_006": 3,
      "comp_genai_157": 2, "comp_web_scraping_158": 2, "comp_mobile_data_159": 2}),
    ("off_intl", "International Cooperation & Statistical Standards Division", "New Delhi",
     ["Ministry of Statistics and Programme Implementation (MoSPI)"], ["SDG-NIF"],
     {"1.2": 6, "1.3": 5, "6.3": 5, "7.2": 6, "7.4": 5, "8.2": 5, "OA.PM": 6, "OA.SM": 12},
     {"comp_intl_standards_133": 4, "comp_intl_coop_180": 4, "comp_sdg_monitor_015": 2,
      "comp_stat_coordination_135": 2, "comp_metadata_sdmx_123": 1}),
    ("off_admin", "Administration, Establishment & Vigilance Division", "New Delhi",
     ["Ministry of Statistics and Programme Implementation (MoSPI)"], [],
     {"7.5": 4, "OA.PM": 16, "OA.FM": 10, "OA.QM": 3},
     {"comp_estab_rules_174": 4, "comp_vigilance_173": 3, "comp_pension_175": 2,
      "comp_audit_response_176": 2, "comp_records_mgmt_179": 2, "comp_procurement_021": 2,
      "comp_public_fin_025": 2}),
]

OFFICE_HEADCOUNT.update({"off_ipmd": 60, "off_dsl": 40, "off_intl": 25, "off_admin": 90})

# Subjects each existing office also owns, now that the families exist.
OFFICE_CORE_EXTRA = {
    "off_nad": {"comp_io_tables_083": 3, "comp_qna_086": 3, "comp_regional_accounts_085": 2,
                "comp_capital_stock_084": 2, "comp_deflators_088": 2, "comp_satellite_acct_087": 1,
                "comp_flow_funds_082": 1, "comp_insurance_stats_065": 1, "comp_capital_mkt_066": 1,
                "comp_construction_stats_062": 1},
    "off_psd": {"comp_producer_price_090": 3, "comp_cost_living_093": 2, "comp_housing_price_091": 2,
                "comp_service_price_092": 1, "comp_ppp_089": 1},
    "off_esd": {"comp_services_stats_060": 3, "comp_msme_stats_061": 3, "comp_business_register_079": 2,
                "comp_trade_stats_059": 2, "comp_ict_stats_068": 1, "comp_ecommerce_stats_069": 1,
                "comp_mining_stats_063": 1},
    "off_isw": {"comp_classification_122": 2, "comp_data_scrutiny_129": 2, "comp_business_register_079": 2,
                "comp_energy_stats_053": 1, "comp_mining_stats_063": 1},
    "off_ssd": {"comp_education_stats_044": 3, "comp_health_stats_045": 3, "comp_gender_stats_047": 3,
                "comp_social_consumption_075": 2, "comp_environment_stats_054": 2,
                "comp_disability_stats_049": 1, "comp_child_stats_048": 1, "comp_elderly_stats_050": 1,
                "comp_migration_stats_073": 1, "comp_crime_stats_071": 1, "comp_governance_stats_072": 1,
                "comp_housing_stats_051": 1, "comp_water_sanitation_052": 1, "comp_nutrition_stats_046": 1,
                "comp_climate_stats_055": 1},
    "off_sdrd": {"comp_labour_stats_041": 4, "comp_informal_sector_043": 2, "comp_timeuse_stats_074": 2,
                 "comp_cost_living_093": 2, "comp_questionnaire_115": 2, "comp_calibration_102": 2,
                 "comp_small_area_101": 2, "comp_var_estimation_106": 2, "comp_panel_survey_118": 1,
                 "comp_mixed_mode_117": 1, "comp_frame_maintenance_119": 1, "comp_sample_reg_077": 1},
    "off_fod_north": {"comp_survey_ops_126": 4, "comp_field_supervision_127": 3, "comp_interview_skills_128": 3,
                      "comp_data_scrutiny_129": 2, "comp_capi_116": 2, "comp_census_pop_078": 1,
                      "comp_agri_census_094": 1},
    "off_fod_east": {"comp_survey_ops_126": 4, "comp_field_supervision_127": 3, "comp_interview_skills_128": 3,
                     "comp_data_scrutiny_129": 2, "comp_capi_116": 2, "comp_census_pop_078": 1,
                     "comp_livestock_stats_095": 1},
    "off_dpd": {"comp_data_eng_136": 3, "comp_dwh_139": 2, "comp_record_linkage_104": 2,
                "comp_imputation_103": 2, "comp_admin_data_125": 2, "comp_data_lake_138": 1,
                "comp_spark_137": 1, "comp_mlops_155": 1},
    "off_capd": {"comp_revision_policy_132": 3, "comp_tabulation_130": 2, "comp_microdata_release_131": 2,
                 "comp_open_data_151": 2, "comp_parliament_171": 2, "comp_media_relations_184": 2,
                 "comp_intl_standards_133": 1},
    "off_dqad": {"comp_nonsampling_120": 3, "comp_outlier_114": 3, "comp_pes_121": 2,
                 "comp_metadata_sdmx_123": 2, "comp_disclosure_105": 2, "comp_classification_122": 1,
                 "comp_admin_data_125": 1},
    "off_nssta": {"comp_training_design_181": 4, "comp_knowledge_mgmt_182": 2, "comp_policy_analysis_188": 1},
}
OFFICES = [(*o[:6], {**o[6], **OFFICE_CORE_EXTRA.get(o[0], {})}) for o in OFFICES]

OFFICE_FUNCTIONAL.update({
    "off_ipmd": ["comp_project_mgmt_024", "comp_public_fin_025", "comp_report_writing_020",
                 "comp_data_viz_019", "comp_risk_mgmt_189", "comp_audit_response_176"],
    "off_dsl": ["comp_python_stats_017", "comp_mlops_155", "comp_data_eng_136", "comp_cloud_infra_027",
                "comp_ai_governance_156", "comp_reproducible_166"],
    "off_intl": ["comp_report_writing_020", "comp_metadata_sdmx_123", "comp_data_gov_016",
                 "comp_project_mgmt_024", "comp_rajbhasha_172"],
    "off_admin": ["comp_records_mgmt_179", "comp_public_fin_025", "comp_procurement_021", "comp_rtt_022",
                  "comp_e_gov_023", "comp_court_cases_177"],
})
for _oid, _extra_functional in {
    "off_nad": ["comp_reproducible_166", "comp_tabulation_130", "comp_metadata_sdmx_123"],
    "off_psd": ["comp_data_eng_136", "comp_web_scraping_158", "comp_tabulation_130"],
    "off_esd": ["comp_metadata_sdmx_123", "comp_excel_adv_163", "comp_tabulation_130"],
    "off_isw": ["comp_excel_adv_163", "comp_tabulation_130", "comp_testing_qa_167"],
    "off_ssd": ["comp_bi_tools_162", "comp_microdata_release_131", "comp_gender_budget_186"],
    "off_sdrd": ["comp_reproducible_166", "comp_stata_164", "comp_testing_qa_167", "comp_metadata_sdmx_123"],
    "off_fod_north": ["comp_excel_adv_163", "comp_records_mgmt_179", "comp_risk_mgmt_189"],
    "off_fod_east": ["comp_excel_adv_163", "comp_records_mgmt_179", "comp_grievance_178"],
    "off_dpd": ["comp_devops_142", "comp_containers_143", "comp_linux_144", "comp_version_ctrl_141",
                "comp_testing_qa_167", "comp_sre_152", "comp_identity_mgmt_147",
                "comp_cybersecurity_ops_146"],
    "off_capd": ["comp_ux_design_150", "comp_social_media_185", "comp_web_dev_148", "comp_knowledge_mgmt_182"],
    "off_dqad": ["comp_testing_qa_167", "comp_reproducible_166", "comp_data_ethics_124"],
    "off_nssta": ["comp_event_mgmt_183", "comp_report_writing_020", "comp_it_procurement_169",
                  "comp_records_mgmt_179"],
}.items():
    OFFICE_FUNCTIONAL[_oid] = list(dict.fromkeys(OFFICE_FUNCTIONAL[_oid] + _extra_functional))

for _tier, _extra_behavioural in {
    "TIER4_JUNIOR": ["comp_attention_detail_200", "comp_time_mgmt_193", "comp_accountability_199",
                     "comp_learning_agility_195", "comp_digital_mindset_202", "comp_teamwork_remote_204"],
    "TIER3_MID": ["comp_accountability_199", "comp_attention_detail_200", "comp_time_mgmt_193",
                  "comp_emotional_int_191", "comp_feedback_205", "comp_public_speaking_203",
                  "comp_teamwork_remote_204", "comp_learning_agility_195"],
    "TIER2_SENIOR": ["comp_coaching_196", "comp_negotiation_192", "comp_conflict_res_197",
                     "comp_systems_think_201", "comp_feedback_205", "comp_resilience_194",
                     "comp_inclusion_198"],
    "TIER1_APEX": ["comp_systems_think_201", "comp_negotiation_192", "comp_inclusion_198",
                   "comp_public_speaking_203", "comp_coaching_196"],
}.items():
    BEHAVIOURAL_BY_TIER[_tier] = list(dict.fromkeys(BEHAVIOURAL_BY_TIER[_tier] + _extra_behavioural))

PRODUCTS.update({
    "Infra-Projects": "Central sector infrastructure project monitoring report",
    "MPLADS": "MPLADS implementation and fund utilisation report",
})
PRODUCT_CRITICAL.update({
    "Infra-Projects": ["comp_project_monitor_100", "comp_construction_stats_062", "comp_project_mgmt_024"],
    "MPLADS": ["comp_mplads_099", "comp_public_fin_025"],
})
for _product, _extra_critical in {
    "NAS": ["comp_io_tables_083", "comp_qna_086", "comp_deflators_088"],
    "CPI": ["comp_cost_living_093"],
    "IIP": ["comp_producer_price_090"],
    "ASI": ["comp_business_register_079"],
    "PLFS": ["comp_labour_stats_041", "comp_calibration_102"],
    "HCES": ["comp_cost_living_093"],
    "SDG-NIF": ["comp_intl_standards_133", "comp_gender_stats_047"],
    "EnviStats": ["comp_environment_stats_054"],
    "Economic Census": ["comp_business_register_079"],
}.items():
    PRODUCT_CRITICAL[_product] = list(dict.fromkeys(PRODUCT_CRITICAL[_product] + _extra_critical))

# Crosswalk rules for the new families. Appended AFTER the original rules, so
# a name the original 40 already claimed keeps its existing mapping (first
# match wins) and only the remainder falls through to these.
CROSSWALK_RULES += [
    (r"labour|employment|workforce|unemploy", "comp_labour_stats_041"),
    (r"\bwage|earnings|remunerat", "comp_wage_stats_042"),
    (r"informal|unorganis|unorganiz", "comp_informal_sector_043"),
    (r"educat|school|literacy|curriculum", "comp_education_stats_044"),
    (r"health|medical|hospital|disease", "comp_health_stats_045"),
    (r"nutrition|food security|\bdiet", "comp_nutrition_stats_046"),
    (r"gender|women|empowerment of women", "comp_gender_stats_047"),
    (r"child|adolescen|youth", "comp_child_stats_048"),
    (r"disabilit|divyang|accessib", "comp_disability_stats_049"),
    (r"elder|ageing|aging|senior citizen", "comp_elderly_stats_050"),
    (r"housing|urban|slum|municipal", "comp_housing_stats_051"),
    (r"water|sanitation|sewer|hygiene", "comp_water_sanitation_052"),
    (r"energy|electricit|power sector|renewable", "comp_energy_stats_053"),
    (r"environment|ecolog|pollution|waste", "comp_environment_stats_054"),
    (r"climate|disaster|emergency|resilience of infrastructure", "comp_climate_stats_055"),
    (r"forest|biodivers|wildlife|natural resource", "comp_forest_stats_056"),
    (r"transport|railway|road|logistic|aviation|shipping", "comp_transport_stats_057"),
    (r"touris|hospitality|travel", "comp_tourism_stats_058"),
    (r"trade|export|import|customs|tariff", "comp_trade_stats_059"),
    (r"\bservices sector|service delivery standards", "comp_services_stats_060"),
    (r"\bmsme\b|small and medium|entrepreneur|startup|start-up", "comp_msme_stats_061"),
    (r"construction|infrastructure development|building", "comp_construction_stats_062"),
    (r"mining|mineral|coal|geolog", "comp_mining_stats_063"),
    (r"bank|monetary|credit|financial inclusion", "comp_banking_stats_064"),
    (r"insurance|pension|provident", "comp_insurance_stats_065"),
    (r"capital market|securit|corporate sector|company affairs", "comp_capital_mkt_066"),
    (r"telecom|spectrum|broadband|connectivity", "comp_telecom_stats_067"),
    (r"\bict\b|digital economy|digital literacy", "comp_ict_stats_068"),
    (r"e-commerce|ecommerce|platform economy", "comp_ecommerce_stats_069"),
    (r"research and development|\br&d\b|patent|scientific research", "comp_rnd_stats_070"),
    (r"crime|police|justice|prosecution|correctional", "comp_crime_stats_071"),
    (r"governance indicator|public administration research", "comp_governance_stats_072"),
    (r"migrat|mobility of workers", "comp_migration_stats_073"),
    (r"time use|unpaid work", "comp_timeuse_stats_074"),
    (r"vital statistic|birth and death|registration of births", "comp_civil_reg_076"),
    (r"business register|enterprise register", "comp_business_register_079"),
    (r"government finance|fiscal statistic|public expenditure statistic", "comp_gfs_080"),
    (r"balance of payments|external sector|foreign exchange", "comp_bop_081"),
    (r"input.output|supply.use", "comp_io_tables_083"),
    (r"state domestic product|regional account|district product", "comp_regional_accounts_085"),
    (r"purchasing power|international comparison", "comp_ppp_089"),
    (r"wholesale price|producer price", "comp_producer_price_090"),
    (r"livestock|fisher|dairy|animal husbandry", "comp_livestock_stats_095"),
    (r"horticultur|plantation", "comp_horticulture_096"),
    (r"irrigation|land use|land record|groundwater|watershed", "comp_irrigation_stats_097"),
    (r"rural development|panchayat|village development", "comp_rural_dev_098"),
    (r"project monitoring|works monitoring|implementation monitoring", "comp_project_monitor_100"),
    (r"small area|domain estimation", "comp_small_area_101"),
    (r"calibrat|weighting|non.response", "comp_calibration_102"),
    (r"imputation|editing of data", "comp_imputation_103"),
    (r"record linkage|entity resolution|data matching", "comp_record_linkage_104"),
    (r"disclosure control|anonymis|anonymiz|de-identif", "comp_disclosure_105"),
    (r"variance estimation|standard error|bootstrap", "comp_var_estimation_106"),
    (r"bayes", "comp_bayesian_107"),
    (r"impact evaluation|causal|counterfactual", "comp_causal_inf_108"),
    (r"experiment|randomis|randomiz|trial design", "comp_exp_design_109"),
    (r"multivariate|factor analysis|clustering", "comp_multivariate_110"),
    (r"econometric|regression analysis", "comp_econometrics_111"),
    (r"forecast|projection|scenario", "comp_forecasting_112"),
    (r"benchmark|reconcil|disaggregation of series", "comp_benchmarking_113"),
    (r"outlier|anomaly|validation rule", "comp_outlier_114"),
    (r"questionnaire|schedule design|cognitive testing", "comp_questionnaire_115"),
    (r"\bcapi\b|\bcati\b|computer assisted", "comp_capi_116"),
    (r"panel survey|longitudinal", "comp_panel_survey_118"),
    (r"sampling frame|frame maintenance", "comp_frame_maintenance_119"),
    (r"non.sampling error|total survey error", "comp_nonsampling_120"),
    (r"post.enumeration|coverage evaluation", "comp_pes_121"),
    (r"classification|\bnic\b|\bnco\b|coicop|taxonom", "comp_classification_122"),
    (r"sdmx|statistical metadata", "comp_metadata_sdmx_123"),
    (r"ethic.* (of|in) data|responsible (use of )?data|data ethics", "comp_data_ethics_124"),
    (r"administrative data|secondary data|registry data", "comp_admin_data_125"),
    (r"field operation|field work|enumeration operation", "comp_survey_ops_126"),
    (r"supervis|inspection of field", "comp_field_supervision_127"),
    (r"interview", "comp_interview_skills_128"),
    (r"scrutiny|manual editing", "comp_data_scrutiny_129"),
    (r"tabulation|table production", "comp_tabulation_130"),
    (r"microdata|unit level data", "comp_microdata_release_131"),
    (r"revision policy|release calendar|dissemination calendar", "comp_revision_policy_132"),
    (r"international standard|global standard|united nations reporting", "comp_intl_standards_133"),
    (r"statistical law|collection of statistics|statistical act", "comp_statistical_law_134"),
    (r"state coordination|centre.state|inter.state coordination", "comp_stat_coordination_135"),
    (r"data engineering|\betl\b|data pipeline", "comp_data_eng_136"),
    (r"spark|hadoop|distributed computing", "comp_spark_137"),
    (r"data lake|lakehouse", "comp_data_lake_138"),
    (r"data warehous|dimensional model", "comp_dwh_139"),
    (r"nosql|mongo|document database", "comp_nosql_140"),
    (r"version control|\bgit\b|source control", "comp_version_ctrl_141"),
    (r"devops|continuous integration|ci/cd", "comp_devops_142"),
    (r"container|docker|kubernetes", "comp_containers_143"),
    (r"linux|shell script|unix", "comp_linux_144"),
    (r"network|system administration|server management", "comp_network_145"),
    (r"incident response|security operation|\bsoc\b", "comp_cybersecurity_ops_146"),
    (r"identity|authenticat|access management|single sign", "comp_identity_mgmt_147"),
    (r"web develop|front.?end|website", "comp_web_dev_148"),
    (r"mobile app|android|\bios\b", "comp_mobile_dev_149"),
    (r"user experience|\bux\b|usability|human centred", "comp_ux_design_150"),
    (r"open data|data portal|data sharing platform", "comp_open_data_151"),
    (r"reliability|uptime|monitoring of services", "comp_sre_152"),
    (r"natural language|text mining|\bnlp\b", "comp_nlp_153"),
    (r"computer vision|image analysis|satellite imagery", "comp_cv_satellite_154"),
    (r"mlops|model lifecycle|model deployment", "comp_mlops_155"),
    (r"ai governance|responsible ai|algorithmic fairness|explainab", "comp_ai_governance_156"),
    (r"generative|large language model|\bllm\b|chatbot", "comp_genai_157"),
    (r"web scraping|crawler|alternative data", "comp_web_scraping_158"),
    (r"sensor|internet of things|\biot\b|mobile network data", "comp_mobile_data_159"),
    (r"synthetic data", "comp_synthetic_data_160"),
    (r"differential privacy|privacy preserving", "comp_dp_161"),
    (r"business intelligence|power bi|tableau", "comp_bi_tools_162"),
    (r"spreadsheet|\bexcel\b", "comp_excel_adv_163"),
    (r"reproducib|workflow automation", "comp_reproducible_166"),
    (r"software testing|quality assurance of software|test automation", "comp_testing_qa_167"),
    (r"requirement|functional specification|business analysis", "comp_req_analysis_168"),
    (r"vendor|outsourc|service provider management", "comp_it_procurement_169"),
    (r"enterprise architecture|solution architecture|interoperability framework", "comp_enterprise_arch_170"),
    (r"parliament|legislative|assurance", "comp_parliament_171"),
    (r"official language|rajbhasha|hindi|translat", "comp_rajbhasha_172"),
    (r"vigilance|disciplinary|conduct rules|corruption", "comp_vigilance_173"),
    (r"establishment|service rules|cadre|recruitment|human resource", "comp_estab_rules_174"),
    (r"pension|retirement|superannuation|gratuity", "comp_pension_175"),
    (r"audit|\bcag\b|accountability of expenditure", "comp_audit_response_176"),
    (r"court|litigation|legal|judicial|\bact\b interpretation", "comp_court_cases_177"),
    (r"grievance|complaint|redress", "comp_grievance_178"),
    (r"record management|file management|archiv|documentation of files", "comp_records_mgmt_179"),
    (r"international cooperation|bilateral|multilateral|diplomat", "comp_intl_coop_180"),
    (r"training|capacity building|learning and development|faculty", "comp_training_design_181"),
    (r"knowledge management|institutional memory|best practice", "comp_knowledge_mgmt_182"),
    (r"event|conference|seminar|workshop organis", "comp_event_mgmt_183"),
    (r"media|press|journalis|public relations", "comp_media_relations_184"),
    (r"social media|outreach|campaign", "comp_social_media_185"),
    (r"gender budget|inclusive budget", "comp_gender_budget_186"),
    (r"monitoring and evaluation|scheme evaluation|programme evaluation", "comp_scheme_eval_187"),
    (r"policy|regulat|legislation drafting", "comp_policy_analysis_188"),
    (r"risk|business continuity|contingency", "comp_risk_mgmt_189"),
    (r"emergency|relief|preparedness", "comp_disaster_mgmt_190"),
    (r"emotional|self.awareness|mindful", "comp_emotional_int_191"),
    (r"negotiat|influenc|persuas", "comp_negotiation_192"),
    (r"time management|productivity|prioritis|prioritiz", "comp_time_mgmt_193"),
    (r"resilien|stress|well.?being|work life", "comp_resilience_194"),
    (r"learning agility|continuous learning|self development", "comp_learning_agility_195"),
    (r"coach|mentor", "comp_coaching_196"),
    (r"conflict", "comp_conflict_res_197"),
    (r"diversity|inclusion|equity|sensitivity", "comp_inclusion_198"),
    (r"accountab|ownership|result orient", "comp_accountability_199"),
    (r"attention to detail|accuracy|thoroughness|meticul", "comp_attention_detail_200"),
    (r"systems thinking|holistic|big picture", "comp_systems_think_201"),
    (r"digital mindset|technology adoption|digital transformation", "comp_digital_mindset_202"),
    (r"public speaking|presentation skill|oratory", "comp_public_speaking_203"),
    (r"remote work|virtual team|cross.functional", "comp_teamwork_remote_204"),
    (r"feedback|appraisal|performance review", "comp_feedback_205"),
]

# Prerequisite edges the new families add. Checked for cycles (together with
# the implicit ladder edges c@L-1 -> c@L) by generate_mock_data.find_cycle.
EXPERT_PREREQUISITES += [
    ("comp_survey_design_002", 2, "comp_calibration_102", 3,
     "Calibration adjusts the design weights a sampling design produces."),
    ("comp_calibration_102", 2, "comp_var_estimation_106", 3,
     "Variance estimation has to account for the weighting steps."),
    ("comp_survey_design_002", 3, "comp_small_area_101", 3,
     "Small area estimators are judged against the direct design-based estimate."),
    ("comp_nat_accounts_001", 2, "comp_io_tables_083", 3,
     "Supply-use balancing is the national accounts production boundary in table form."),
    ("comp_nat_accounts_001", 3, "comp_qna_086", 3,
     "Quarterly accounts extrapolate the annual national accounts."),
    ("comp_index_numbers_004", 2, "comp_deflators_088", 3,
     "Deflation is index-number theory applied to volume measurement."),
    ("comp_price_stats_003", 2, "comp_producer_price_090", 3,
     "Producer price indices reuse the consumer price collection machinery."),
    ("comp_trade_stats_059", 2, "comp_bop_081", 3,
     "The current account is built on merchandise trade statistics."),
    ("comp_gfs_080", 2, "comp_flow_funds_082", 4,
     "The government sector's financial account comes from government finance statistics."),
    ("comp_environment_stats_054", 3, "comp_satellite_acct_087", 4,
     "Environmental accounts are the largest satellite account in practice."),
    ("comp_labour_stats_041", 2, "comp_informal_sector_043", 3,
     "Informal employment is defined on the labour force framework."),
    ("comp_db_design_029", 2, "comp_data_eng_136", 3,
     "Pipelines move data between modelled tables."),
    ("comp_data_eng_136", 2, "comp_dwh_139", 3,
     "A warehouse is loaded by pipelines."),
    ("comp_python_stats_017", 2, "comp_nlp_153", 3,
     "Text processing work is taught in Python."),
    ("comp_ml_stats_005", 3, "comp_mlops_155", 3,
     "You cannot operate models you cannot build."),
    ("comp_questionnaire_115", 2, "comp_capi_116", 3,
     "A CAPI instrument encodes a designed questionnaire."),
    ("comp_survey_ops_126", 2, "comp_field_supervision_127", 3,
     "Supervision presupposes knowing how the round is organised."),
    ("comp_econometrics_111", 3, "comp_causal_inf_108", 3,
     "Causal designs are estimated with regression machinery."),
    ("comp_data_privacy_026", 2, "comp_disclosure_105", 3,
     "Disclosure control is the statistical arm of confidentiality protection."),
    ("comp_disclosure_105", 3, "comp_dp_161", 4,
     "Formal privacy is judged against traditional disclosure control."),
    ("comp_admin_data_125", 2, "comp_record_linkage_104", 3,
     "Linkage is how administrative sources are joined to survey data."),
    ("comp_cloud_infra_027", 2, "comp_containers_143", 3,
     "Containers are scheduled on cloud infrastructure."),
    ("comp_version_ctrl_141", 2, "comp_devops_142", 3,
     "Delivery pipelines are triggered from version control."),
    ("comp_comm_032", 2, "comp_public_speaking_203", 3,
     "Presenting is communication in front of an audience."),
    ("comp_leadership_032", 3, "comp_coaching_196", 4,
     "Coaching is a leadership practice, not a separate craft."),
]
